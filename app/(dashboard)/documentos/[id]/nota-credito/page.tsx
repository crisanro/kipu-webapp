...
import {
  ArrowLeft, Loader2, CheckCircle2, AlertTriangle, Trash2
} from "lucide-react";
...
export default function NotaCreditoPage() {
  ...
  const emitir = async () => {
    ...
    if (!empresa?.suscripcion_activa) { setError("Se requiere suscripción activa."); return; }

    setSubmitting(true);
    try {
      // ← ruta actualizada
      const res = await api.post("/api/v1/app/documentos/emit/NCR", {
        doc_origen_id: id,
    ...
    <div className="p-4 md:p-6 max-w-3xl mx-auto space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          {/* ← numero_doc en vez de numero_factura */}
          <p className="text-sm text-gray-500">Documento original: {factura.numero_doc}</p>
        </div>
      </div>
      ...
      <div className="flex items-start gap-3 bg-amber-500/10 border border-amber-500/20 rounded-xl px-4 py-3">
        ...
        <div>
          ...
          <p className="text-xs text-amber-400/70 mt-0.5">
            Para anular completamente deja todos los ítems con sus cantidades originales.
          </p>
        </div>
      </div>

      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
        <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Documento original</h2>
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div>
            <p className="text-xs text-gray-500">Cliente</p>
            <p className="text-white font-medium">
              {info.razonSocialComprador || datos.legacy_razon_comprador || factura.cliente?.razon_social}
            </p>
            <p className="text-xs text-gray-500">
              {info.identificacionComprador || datos.legacy_id_comprador || factura.cliente?.identificacion}
            </p>
          </div>
          <div>
            <p className="text-xs text-gray-500">Fecha emisión</p>
            <p className="text-white">{info.fechaEmision || factura.fecha_emision}</p>
            <p className="text-xs text-gray-500">Total: ${fmt(parseFloat(factura.importe_total ?? 0))}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
        ...
        <input
          ...
          placeholder="Ej: DEVOLUCION DE BIEN, ANULACION DE COMPROBANTE..."
        ...
        <p className="text-xs text-gray-600 mt-1">
          Motivos: DEVOLUCION DE BIEN · ANULACION DE COMPROBANTE · REBAJA DE PRECIO · OTROS
        </p>
      </div>

      <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-800">
          <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Ítems a acreditar</h2>
            ...
            return (
              <div key={item._id} className="p-4 space-y-3">
                <div className="flex items-center gap-2">
                  <span className="text-xs text-gray-600 w-5 shrink-0 text-center">#{index + 1}</span>
                  <input
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                  <div className="space-y-1">
                    <label className="text-[10px] uppercase font-semibold tracking-wider text-gray-500 block">Cantidad</label>
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] uppercase font-semibold tracking-wider text-gray-500 block">Precio Unit.</label>
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] uppercase font-semibold tracking-wider text-gray-500 block">IVA</label>
                    <select value={item.tipo_iva} onChange={(e) => editItem(item._id, "tipo_iva", e.target.value)}
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] uppercase font-semibold tracking-wider text-gray-500 block">Total</label>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
        <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Resumen NC</h2>
      ...
      <div className="flex gap-3 pb-6">
        ...
        <button onClick={emitir} disabled={submitting || !empresa?.suscripcion_activa}
